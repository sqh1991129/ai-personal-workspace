...
def register_exception_handler(app: FastAPI) -> None:
    ...
    async def custom_exception_handler(request: Request, exc: AppException):
        ...
        return JSONResponse(
            ...
            content=BaseResponse.failure(code=exc.code, message=exc.message, data=exc.data).model_dump())
    ...
    async def validation_exception_handler(request: Request, exc: RequestValidationError):
        ...
        return JSONResponse(
            ...
            content=BaseResponse.failure(code=40000, message=f"参数校验失败: {detail_msg}").model_dump()
        )
    ...
    async def http_exception_handler(request: Request, exc: HTTPException):
        ...
        return JSONResponse(
            ...
            content=BaseResponse.failure(code=exc.status_code, message=str(exc.detail)).model_dump()
        )
    ...
    async def exception_handler(request: Request, exc: Exception):
        ...
        # .model_dump() 将创建好的 BaseResponse 模型对象（Model Object） 导出/转换为 Python 原生的 字典（dict）
        return JSONResponse(
            status_code=status.HTTP_200_OK,
            content=BaseResponse.failure(code=ErrorCodes.SYSTEM_ERROR.code, message=ErrorCodes.SYSTEM_ERROR.message).model_dump()
        )
